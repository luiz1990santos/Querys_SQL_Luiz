-- select * from `eai-datalake-data-sandbox.analytics_prevencao_fraude.Tb_Movimentacao_InOut_Acima_Limite_V2`

create or replace table `eai-datalake-data-sandbox.analytics_prevencao_fraude.Tb_Movimentacao_InOut_Acima_Limite_V2` as
with
...
WHERE 
date(pix.created_at) > '2024-03-01'
...
group by 
  ...
  case 
    when valor_limite.CPF_CNPJ_Destino = ins.cpf_completo then 'Sim'
    else 'Não'
...
ON valor_limite.CPF_CNPJ_Destino = bio2.CPF_Cliente
left join base_Inspetoria as ins
ON valor_limite.CPF_CNPJ_Origem = ins.cpf_completo
left join base_Inspetoria as ins2
ON valor_limite.CPF_CNPJ_Destino = ins2.cpf_completo
--WHERE 
  -- valor_limite.OPERACAO = 'PIX-OUT' and
  -- valor_limite.CPF_CNPJ_Origem = '71362584487'
  -- AND type = 'CASH-IN'