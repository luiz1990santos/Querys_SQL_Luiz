-- select * from `eai-datalake-data-sandbox.business_monitoring.Tb_Alerta_Pixin_AcimaDe1000`

--create or replace table `eai-datalake-data-sandbox.business_monitoring.Tb_Alerta_Pixin_AcimaDe1000` as
with
...
WHERE 
date(pix.created_at) >= current_date() - 15
...
,case 
    when cl_CPF_CNPJ_Origem.status is null then 'Não tem Conta' 
    else cl_CPF_CNPJ_Origem.status
 end as StatusContaOrigem
,cl_CPF_CNPJ_Destino.status as StatusContaDestino
,flow
,type
...
where valor > 1000
              ...
              select 
                  *, 
                  LPAD(CAST(CPF AS STRING), 11, '0') AS cpf_completo  
              from `eai-datalake-data-sandbox.analytics_prevencao_fraude.Tb_Base_Inspetoria_2`
            

) select 
    distinct
      pixin.Dt_Transacao,
      pixin.Dt_Abert_Origem,
      pixin.Temp_Conta_Origem,
      pixin.CPF_CNPJ_Origem,
      pixin.Origem_Nome,
      pixin.StatusContaOrigem,
      pixin.Origem_Banco,
      pixin.Dt_Abert_Destino,
      pixin.Temp_Conta_Destino,
      pixin.CPF_CNPJ_Destino,
      pixin.Destino_Nome,
      pixin.StatusContaDestino,
      pixin.Destino_Banco,
      pixin.Flag_Titularidade,
      pixin.OPERACAO,
      pixin.codigo_transacao,
      pixin.Valor,
      case 
        when pixin.CPF_CNPJ_Origem = ins.cpf_completo then 'Sim'
        else 'Não'
      end as Flag_Inspetoria_Origem,
      case 
        when pixin.CPF_CNPJ_Destino = ins.cpf_completo then 'Sim'
        else 'Não'
...
from base_Consolidada AS pixin
left join base_Bio AS bio 
ON pixin.CPF_CNPJ_Origem = bio.CPF_Cliente
left join base_Bio AS bio2 
ON pixin.CPF_CNPJ_Destino = bio2.CPF_Cliente
left join base_Inspetoria as ins
ON pixin.CPF_CNPJ_Origem = ins.cpf_completo
left join base_Inspetoria as ins2
ON pixin.CPF_CNPJ_Destino = ins2.cpf_completo
WHERE pixin.OPERACAO = 'PIX-IN'
--and (pixin.StatusContaOrigem = 'Não tem Conta' or pixin.StatusContaOrigem = 'ACTIVE')
--and pixin.StatusContaDestino = 'ACTIVE'
order by pixin.Valor desc
--and pixin.CPF_CNPJ_Origem = '98030442653'
