CREATE OR REPLACE TABLE `eai-datalake-data-sandbox.analytics_prevencao_fraude.Tb_Prevencao_Alerta_PixIn_1000` AS 
...
WHERE DATE(cash_transaction.created_at) >= CURRENT_DATE - INTERVAL 1 DAY
  AND DATE(cash_transaction.created_at) <= CURRENT_DATE
...
,a.valor
,codigo_transacao
,DATETIME(TIMESTAMP(CURRENT_DATETIME()), 'America/Sao_Paulo') AS DataUltimaAtualizacao
from base_Consolidada as a
where valor >= 1000
and OPERACAO in ('PIX-IN', 'TED-IN')
